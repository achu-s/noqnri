...
function get_array_values($arr){
	...
	foreach ($arr as $key=>$val){
		...
		$image = ($val["image"]!=NULL)?'<img class="fixd-25-img" src="'.Common::backend_url().'/uploads/category/'.$val["image"].'">':'<img class="fixd-25-img" src='.Yii::app()->baseurl.'/images/25X25.png>';
  		...
  		?>
			...
			if (Partner::model()->check_last_child($categories->id )) {
				...
				<?php if($categories["category_image"]!=NULL){?>
					<li id="<?php echo $i;?>" class='<?php echo $class;?>'><a href="javascript:void(0);" ref="<?php echo $categories['category'];?>" id="<?php echo $categories['id'];?>"><img class="fixd-25-img" src="<?php echo Common::backend_url();?>/uploads/category/<?php echo $categories["category_image"];?>"> <?php echo $categories['category'];?><?php echo $side_icon;?></a>
				<?php }else{?>
		    	 ...
		    	 $.ajax({
		 		    ...
		 		    success: function(result) 
		 		    {
			 		    if(result==0){
		 		    		...
		 		    		$('.header-hospitals-main').css({'background-image': 'url(<?php echo Common::backend_url();?>/uploads/category/banner/'+result+')'});return false;
				 		}
		 			}
		 		});
			}
