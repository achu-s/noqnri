...
class Card extends CActiveRecord
{
	...
	public function Status($data){
	    ...
	    echo "<input type='checkbox' class='switch' ".$status." ref='$data->id' res='Category'>";
	}
	public function CardNumber($data){
	    if($data->card_number){
	        $LoginData = Login::model()->findByAttributes(array('username'=>$data->card_number));
	        if($LoginData){
	            $customerdata = Customer::model()->findByAttributes(array('login_id'=>$LoginData->id));
	            if($customerdata){
        	        if($data->card_issue_status!='Pending'){
        	            echo "<a href=".Yii::app()->baseUrl."/customer/customerView/".$customerdata->id.">".$data->card_number."</a>";
        	        }else{
	...
	public function ActionButton($data){
	    if($data->card_issue_status=="Pending" && $data->phone_number!=NULL){
	        echo '<a class="btn btn-success btn-xxs" title="Update" style="margin-right:0px"><i class="icon-pencil icon-white"></i> Assigned</a>';
	    }else{
	        echo '<a class="btn btn-primary btn-xxs" title="Update" style="margin-right:0px" data-toggle="modal" data-target="#myModal" onclick="editCard(this)" id='.$data->id.'><i class="icon-pencil icon-white"></i> Edit</a>';
	    }
	}
}
