...
class SiteController extends Controller
{
    ...
    public function actionIndex() {
        ...
        $exclusive_privileges = Category::model()->findAllByAttributes(array('parent_category'=>'0','status'=>'Y'),array('order' => 'created_at asc','limit' => 8));
        $totalCardholders = Card::model()->countByAttributes(array('card_issue_status'=>'Approved'));
        $totalPartners = Partner::model()->countByAttributes(array('status'=>'Y'));
        $privillageList = PrivillageUpdates::model()->findAllByAttributes(array('status'=>'Y'));
        $testimonialList = NewsTestimonial::model()->findAllByAttributes(array('status'=>'Y'));
        $homeBanners = HomeBanner::model()->findAllByAttributes(array('status'=>'Y'));
        $news_events = NewsTestimonial::model()->findAllByAttributes(array('is_testimonial'=>'0'));
        $customer_testimonials = NewsTestimonial::model()->findAllByAttributes(array('is_testimonial'=>'1','user_type'=>'0'));
        $partner_testimonials = NewsTestimonial::model()->findAllByAttributes(array('is_testimonial'=>'1','user_type'=>'1'));
        $enquiry = new Enquiry();
        if($_POST['Enquiry']){
        ...
        $this->render('//site/index',array('exclusive_privileges'=>$exclusive_privileges,'enquiry'=>$enquiry,'totalCardholders'=>$totalCardholders,'totalPartners'=>$totalPartners,'privillageList'=>$privillageList,'testimonialList'=>$testimonialList,'homeBanners'=>$homeBanners,'news_events'=>$news_events,'customer_testimonials'=>$customer_testimonials,'partner_testimonials'=>$partner_testimonials));
    }
